...
class MediaConst {
    ...
    const E_NOT_AVAILABLE = 1;
    const E_INVALID_ARGUMENT = 2;
    const E_INVALID_FILE = 3;
    
    /**
     * Graphics error constants.
     */
    const E_UNSPECIFIED = -1;
    const E_NOT_LOADED = 0x280; // no file is loaded
    const E_NO_LIBRARY = 0x281; // the image function library is not available. Check php.ini for included image library extension
    const E_NOT_IMAGE = 0x282; // the data is corrupted or is not a supported image format
    const E_FILE_EMPTY = 0x283; // the file is empty
    const E_NOT_CORRECT_IMAGE_TYPE = 0x284; // cannot create image from file as the file is the wrong type or is corrupt
    const E_BAD_DEST_FILE_PARAM = 0x285; // invalid destination file parameter
    const E_BAD_SOURCE_FILE_PARAM = 0x286; // invalid source file parameter
    const E_BAD_SOURCE_FILE_PROPERTY = 0x287; // invalid source file property
    const E_BAD_TYPE = 0x288; // invalid image type
    const E_FILE_EXISTS = 0x289; // file exists and overite not allowed
    const E_FILE_NOT_FOUND = 0x28a; // file not found
    const E_IMAGE_SIZE_INVALID = 0x28b; // invalid image dimensions
    const E_INVALID_ALIGN_PARAM = 0x28c; // invalid image alignment parameter
    const E_INVALID_ROTATE_PARAM = 0x28d; // invalid image rotation parameter
    const E_INVALID_SIZE_PARAM = 0x28e; // invalid image size parameter
    const E_IPTC_CANNOT_EMBED = 0x28f; // cannot embed IPTC code
    const E_IPTC_CANNOT_OPEN = 0x290; // cannot open image file
    const E_IPTC_CANNOT_WRITE = 0x291; // cannot write IPTC code to image file
    const E_IPTC_INVALID_IPTC_FIELD = 0x292; // invalid IPTC embedding code
    const E_IPTC_NO_FUNCTION = 0x293; // IPTC functions are not enabled
    const E_NOT_INITIALIZED = 0x294; // no image file is loaded
    const E_NO_FUNCTION = 0x295; // cannot create image as image function '%s' is not available
    const E_NO_OVERLAY_IMAGE = 0x296; // no overlay image specified in the overlayImage property
    const E_OVERLAY_TRANSPARENT = 0x297; // overlay image transparent color cannot be set
    const E_PARAMETER_INVALID = 0x298; // invalid function parameter
    const E_PHP_FUNCTION_FAILED = 0x299; // a PHP image function has failed
    const E_TYPE_MISMATCH = 0x29a; // type mismatch
    const E_PROPERTY_NOT_FOUND = 0x29b; // class property not found
    const E_MEMORY = 0x29c; // browser script memory error
    ...
    const T_BAD_DEST_FILE_PARAM = "invalid destination file parameter";
    const T_BAD_SOURCE_FILE_PARAM = "invalid source file parameter";
    const T_BAD_SOURCE_FILE_PROPERTY = "invalid source file property";
    const T_BAD_TYPE = "invalid image type";
    const T_FILE_EXISTS = "file exists and overite not allowed";
    const T_FILE_NOT_FOUND = "file not found: '%s'";
    const T_IMAGE_SIZE_INVALID = "invalid image x(%s) and/or y(%s) parameters";
    const T_INVALID_ALIGN_PARAM = "invalid image alignment parameter";
    const T_INVALID_ROTATE_PARAM = "invalid image rotation parameter";
    const T_INVALID_SIZE_PARAM = "invalid image size parameter";
    const T_IPTC_CANNOT_EMBED = "cannot embed IPTC code";
    const T_IPTC_CANNOT_OPEN = "cannot open image file";
    const T_IPTC_CANNOT_WRITE = "cannot write IPTC code to image file";
    ...
    const T_OVERLAY_TRANSPARENT = "overlay image transparent color cannot be set";
    const T_PARAMETER_INVALID = "invalid '%s' parameter '%s'";
    const T_PHP_FUNCTION_FAILED = "image function '%s' failed";
    const T_TYPE_MISMATCH = "type mismatch";
    const T_PROPERTY_NOT_FOUND = "property not found: '%s'";
    const T_MEMORY = "browser script memory error";    
