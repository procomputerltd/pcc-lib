...
class Arrays {
     ...
     * @param mixed $default (optional) Value returned when $mixed cannot be converted.
     * @return array|mixed
     */
    public static function toArray(mixed $mixed, mixed $default = []) {
        if(is_array($mixed)) {
            return $mixed;
        }
        if(null === $mixed) {
        ...
        if($mixed instanceof \Traversable) {
            return iterator_to_array($mixed);
        }
        if(is_object($mixed)) {
            if(method_exists($mixed, 'getArrayCopy')) {
                return $mixed->getArrayCopy();
            }
            // Check for objects having 'toArray()' method.
